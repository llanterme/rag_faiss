...
try:
    ...
    logger.warning(
        "unstructured not available. Advanced layout analysis will be limited."
    )
...
class EnhancedDocumentProcessor:
    ...
    def process_pdf(self, path: Path) -> List[Document]:
        ...
        if (
            ...
            and (self.extract_tables or self.preserve_formatting)
            and not settings.disable_unstructured_ui
        ):
            try:
    ...
    def _split_documents(self, documents: List[Document]) -> List[Document]:
        ...
        for doc in documents:
            ...
            if (
                ...
                or len(doc.page_content) <= self.chunk_size
            ):
